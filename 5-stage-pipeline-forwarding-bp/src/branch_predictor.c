...
enum {
    ...
    BTB_SIZE = 1<<(BTB_INDEX_BITS + BTB_HISTORY_BITS),
...
void bp_branch_taken(word_t pc, word_t target, int delayed) {
    ...
    history = ((history << 1) | 1) & ((1 << BTB_HISTORY_BITS) - 1);
}
...
void bp_branch_not_taken(word_t pc, word_t target, int delayed) {
    ...
    history = ((history << 1) | 0) & ((1 << BTB_HISTORY_BITS) - 1);
}
...
static word_t get_index(word_t pc) {
    return (history << BTB_INDEX_BITS) | BITS(pc, 0, BTB_INDEX_BITS-1);
    //return history ^ BITS(pc, 0, BTB_INDEX_BITS-1);
}
