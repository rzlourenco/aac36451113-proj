...
void ex_stage(void) {
    ...
    } else if (ex_state.br_pred_taken) {
        cpu_state.id_enable = 0;
        if_state.pred_delay_slot = 0;
        cpu_state.pc = ex_state.pc + 4;
    }
}
...
static void branch_control(word_t branch_op, word_t alu_result) {
    ...
    word_t const target = alu_result;
    int const taken = (ex_state.branch_cond == EX_COND_ALWAYS)
    ...
    int failed =
            taken != ex_state.br_pred_taken
            || target != ex_state.br_pred_target
            || ex_state.branch_delayed != ex_state.br_pred_delayed;

    if (taken)
        bp_branch_taken(ex_state.pc, target, ex_state.branch_delayed);
    else
        bp_branch_not_taken(ex_state.pc, target, ex_state.branch_delayed);

    if (failed) {
        cpu_state.id_enable = 0;
        if_state.pred_delay_slot = 0;

        if (ex_state.branch_delayed) {
            cpu_state.pc = ex_state.pc + 4;
            if_state.next_pc = ex_state.pc + 8;

            if (taken) {
                if_state.pred_delay_slot = 1;
                if_state.branch_pc = target;
            }
        } else if (taken) {
            cpu_state.pc = target;
        } else {
        ...
        cpu_state.branches_predicted += 1;
    }

    if (taken && target == ex_state.pc) {
        cpu_state.if_enable = 0;
    }

    cpu_state.branches += 1;
}
